...
#else
    // On Linux ARM64 with GCC < 10, use a null logger to avoid filesystem
    // operations
...
#else
  GTEST_SKIP()
    << "Multi-sink filesystem tests disabled on Linux ARM64 with GCC < 10";
#endif
...
TEST_F(SpdlogTest, DISABLED_AsyncLogging)
{
  ...
  // Note: Not asserting on file contents as async logging might not be reliable
  // in tests
}
